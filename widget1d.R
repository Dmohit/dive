widget.env <- function()
  {
    ...
    chooseProjections <- function(h,...)
      {
        ...
        yes <- gbutton("OK",
                       ...
                       handler=function(h,...)
                       {
                         ...
                         else projs <<- unique(c(projs,svalue(projtab)))
    ...
    createTab <- function(NoteBookHandle,GGobiHandle,TabName,lst)
      {
        ...
        buttons.frame <- gframe(horizontal=TRUE,container=newpage)
        info.frame <- gframe(horizontal=TRUE,container=newpage)
        #Variables Frame
        glabel("Display Variables are X1 and Y1", container = variables.frame)
        glabel("Output Variables are X7 and Y7", container = variables.frame)
        #Brushes Frame
        ...
        for(i in 1:length(allcols))
          {
#            pair <- GGobi.brush.to.pair(GGobiHandle,lst=lst,color=brushes[i])
        ...
        sync <- gbutton("Sync Output and Drawing Window",container=glob.buts)
        save.proj <- gbutton("Save Current Projection Matrix",container=glob.buts)
        #Information Frame
        obj5 <- gtext("This current view has moved points",container=info.frame,expand=TRUE,height=50)
        updateTab <- function()
          {
        ...
        for(i in 1:numviews)
          {
            ...
            tabs[[i]] <<- createTab(nb,GGobiHandle,as.character(i),lst=view[[i]])
          }
      }
